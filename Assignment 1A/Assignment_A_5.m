...
EOMtf = subs(EOMtf, {F2, F3}, {0, 0});  % F2 & F3 are set to zero
...
c = 0.1e0;
EOMtf = subs(EOM, [m1 m2 m3 c1 c2 c3 k1 k2 k3], [2 0.2 0.05 c c c 1e4 3e4 4e4]);
EOMtf = subs(EOMtf, {F1, F3}, {0, 0});  % F1 & F3 are set to zero
...
% Extract x in terms of F2
x1_sol = sol.x1;
x2_sol = sol.x2;
x3_sol = sol.x3;

% Now, compute the transfer function H(s) = x/F2
...
%% Defining Parameters for the transfer function x/F3 (wrt 3rd Actuator Force)
c = 0.1e0;
EOMtf = subs(EOM, [m1 m2 m3 c1 c2 c3 k1 k2 k3], [2 0.2 0.05 c c c 1e4 3e4 4e4]);
EOMtf = subs(EOMtf, {F1, F2}, {0, 0});  % F1 & F2 are set to zero

% Create the system of equations
eqs = EOMtf;

% Solve for x1, x2, and x3 in terms of F3
sol = solve(eqs, [x1, x2, x3]);

% Extract x in terms of F3
x1_sol = sol.x1;
x2_sol = sol.x2;
x3_sol = sol.x3;

% Now, compute the transfer function H(s) = x/F3
H_x1F3 = simplify(x1_sol / F3);
H_x2F3 = simplify(x2_sol / F3);
H_x3F3 = simplify(x3_sol / F3);

% Extract the coefficients of the numerator and denominator:
[num, den] = numden(simplify(x1_sol / F3));
[num2, den2] = numden(simplify(x2_sol / F3));
[num3, den3] = numden(simplify(x3_sol / F3));

% Now, you can create the transfer function using the coefficients:
H_x1F3 = tf([sym2poly(num)], [sym2poly(den)]);
H_x2F3 = tf([sym2poly(num2)], [sym2poly(den2)]);
H_x3F3 = tf([sym2poly(num3)], [sym2poly(den3)]);

figure(3);clf(3);
bode(H_x1F3,H_x2F3,H_x3F3,opts)
legend
