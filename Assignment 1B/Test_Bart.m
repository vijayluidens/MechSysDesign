...
% Plot frequency response and coherence in Figure 2
figure(2);clf(2);
...
subplot(3,1,2);semilogx(f,rad2deg(unwrap(angle(T)))); grid on; hold on;
...
iodelay = 00e-6; % Tune delay 
sys = tfest(data,np,nz,iodelay);
Pnump = sys.Numerator;
Pdenp = sys.Denominator;
Ptf = tf(Pnump,Pdenp);

% Plot Bode Plot of the Transfer Function in Figure 2 (same figure)

[mag, phase, f] = bode(Ptf, f);
phase=phase-720
% Plot the magnitude and phase from the Bode plot into the first two subplots
subplot(3,1,1);
semilogx(f, mag2db(squeeze(mag)), 'r'); % Plot magnitude in dB
title('Magnitude of Frequency Response');
xlabel('Frequency (Hz)');
ylabel('Magnitude (dB)');

subplot(3,1,2);
semilogx(f, squeeze(phase),'r'); % Plot phase in degrees
title('Phase of Frequency Response');
xlabel('Frequency (Hz)');
ylabel('Phase (degrees)');
